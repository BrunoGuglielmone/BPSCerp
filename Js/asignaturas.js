document.addEventListener('DOMContentLoaded', () => {
    ...
    const form = document.getElementById('registro-asignatura-form');
    ...
    const hiddenId = document.getElementById('asignatura_id');
    let dataCache = [];
    ...
    const renderizarTabla = (datos) => {
        tbody.innerHTML = '';
        if (datos.length === 0) {
            tbody.innerHTML = `<tr><td colspan="3">No se encontraron asignaturas.</td></tr>`;
            return;
        }
        datos.forEach(item => {
            const fila = `
    ...
    const eliminar = async (ids) => {
        if (!confirm(`¿Eliminar ${ids.length} asignatura(s)?`)) return;
        const response = await fetch(API_URL, {
            method: 'DELETE',
